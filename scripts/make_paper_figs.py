...
RESULTS_PATH = "outputs/c_check_tuned_ext2/results_e1_prev.csv"
# Color version output directory.
...
METHOD_MARKERS = {
    "sahdpca": "o",             # fb
    "sahdpca_wo_feedback": "s", # no-fb
    "sahdpca_strong": "^",      # strong
}

# Colorblind-friendly palette.
PALETTE = {
    "sahdpca": "#1b9e77",          # teal/green
    "sahdpca_wo_feedback": "#d95f02",  # orange
    "sahdpca_strong": "#7570b3",       # purple
}
...
def load_results(results_path: str) -> pd.DataFrame:
    ...
    # The archive may contain repeated runs for the same key after reruns.
    # Keep the latest record per (dataset, method, eps, seed) for paper figures.
    dedup_key = ["dataset", "method", "eps_tot", "seed"]
    if all(col in df.columns for col in dedup_key):
        df = df.reset_index(drop=False).rename(columns={"index": "__rowid"})
        df = (
            df.sort_values("__rowid")
            .drop_duplicates(subset=dedup_key, keep="last")
            .drop(columns=["__rowid"])
...
def main() -> None:
    ...
    # Fig.1: cumulative budget (eps=1, seed=0).
    ...
    # Fig.2: per-iteration eps schedule (eps=1, seed=0).
    plot_eps_schedule(
        df,
        datasets=["har", "gas"],
        eps=1.0,
    ...
    # Fig.3: noise scale over iterations (eps=1, seed=0).
    plot_noise_scale(
        df,
        datasets=["har", "gas"],
        eps=1.0,
    ...
    # Fig.4: privacy–utility (F1 vs epsilon).
    plot_metric_vs_eps(
        df,
        metric="f1",
    ...
    # Fig.5: reconstruction error (SSE_x vs epsilon).
    plot_metric_vs_eps(
        df,
        metric="sse_x",
