...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run recent baselines for eps=0.8/1.0 and reuse historical results if available.")
...
def pick_history_path(user_path: Optional[str]) -> Optional[Path]:
    ...
    for path in candidates:
        ...
        if {"SA-HDPCA(fb)", "PCA-DP"}.issubset(set(df["method"].unique())):
...
def main() -> None:
    ...
    if history_df is not None:
        ...
        method_filter = {"SA-HDPCA(fb)", "PCA-DP"}
    ...
    for ds in datasets:
        ...
        for eps_tot in eps_list:
            ...
            for seed in seeds:
                ...
                for method in ["SA-HDPCA(fb)", "PCA-DP"]:
    ...
    def run_task(task: Tuple[str, float, int, str]) -> Dict:
        ...
        if args.use_full_method and method in {"SA-HDPCA(fb)", "PCA-DP"}:

            if full_config is None:

                raise RuntimeError("Full config not loaded for full method run.")

            method_map = {"SA-HDPCA(fb)": "sahdpca", "PCA-DP": "pca_dp"}

            core_method = method_map[method]
