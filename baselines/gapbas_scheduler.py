...
# GAPBAS (SSRN 4567891) Algorithm 2 mapping:
# 1) kmeans++ init -> kmeanspp_init in _evaluate_schedule
# 3-4) GA schedule population under sum/eps_min constraints -> optimize_schedule()
# 5-7) DP k-means with eps_schedule -> dp_kmeans call in _evaluate_schedule
# 8-11) fitness + selection/crossover/mutation -> GA loop in optimize_schedule
# Simplifications: fixed T/eps_min and SSE proxy instead of NICV.
...
def _project_schedule(raw: np.ndarray, eps_tot: float, eps_min: float, eps_max: float) -> np.ndarray:
    ...
    # enforce exact sum by adjusting last element
...
def optimize_schedule(
    ...
    # optional subsample to reduce GA runtime
    ...
    # final evaluation on full Z for the best schedule
